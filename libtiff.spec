...
%ifarch sparc64
mkdir -p sparc32
cp libtiff/.libs/libtiff.so sparc32/
make clean
...
CC="cc"
CXX="CC"
export LD_LIBRARY_PATH LD_RUN_PATH LD CC CXX LDFLAGS
./configure < /dev/null   # interactive no more!
...
install -m 0444 libtiff/.libs/libtiff.a $RPM_BUILD_ROOT/usr/local/lib
/usr/ccs/bin/strip libtiff/.libs/libtiff.so

%ifarch sparc64
/usr/ccs/bin/strip sparc32/libtiff.so
mkdir -p $RPM_BUILD_ROOT/usr/local/lib/sparcv9
install -m 0555 libtiff/.libs/libtiff.so $RPM_BUILD_ROOT/usr/local/lib/sparcv9
install -m 0555 sparc32/libtiff.so $RPM_BUILD_ROOT/usr/local/lib
%else
install -m 0555 libtiff/.libs/libtiff.so $RPM_BUILD_ROOT/usr/local/lib
%endif
...
%changelog
* Wed Aug 29 2007 David Lee Halik <dhalik@nbcs.rutgers.edu> - 3.8.2-1
- Bump to 3.8.2
- Switched to SunCC
