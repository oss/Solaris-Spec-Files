...
%description
Web application addon for creating a users public_html directory and putting down a sample index.html file
...
mkdir -pm 0755 $RPM_BUILD_ROOT%{prefix}/webtools/etc/public-html

install -c -m 0555 $RPM_BUILD_DIR/%{name}-%{version}/src/copy $RPM_BUILD_ROOT%{prefix}/%{name}-%{version}/webbin/
install -c -m 0555 $RPM_BUILD_DIR/%{name}-%{version}/src/makedir $RPM_BUILD_ROOT%{prefix}/%{name}-%{version}/webbin/
...
%dir %{prefix}/webtools/etc/public-html
