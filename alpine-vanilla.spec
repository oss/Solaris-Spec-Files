...
# We shouldn't need these anymore. We're on version 2.02
...
CXX="CC" \
LD="/usr/ccs/bin/ld" \
LDFLAGS="-L/usr/local/lib -R/usr/local/lib -L/usr/local/ssl/lib -R/usr/local/ssl/lib -llber -lnsl -lsocket -Bdirect -lintl" \
CFLAGS="-g -xs -I/usr/local/include " \
CPPFLAGS="-g -xs -I/usr/local/include "
export PATH CC CXX CPPFLAGS LD LDFLAGS CFLAGS

#LDFLAGS="-L/usr/local/lib -R/usr/local/lib -L/usr/local/ssl/lib -R/usr/local/ssl/lib -llber -lnsl -lsocket -Bdirect -zdefs -lintl" \
...
%{_topdir}/BUILD/re-alpine-%{version}/web/bin/alpined
%{_topdir}/BUILD/re-alpine-%{version}/web/bin/alpineldap
%{_topdir}/BUILD/re-alpine-%{version}/web/lib/libwpcomm.a
%{_topdir}/BUILD/re-alpine-%{version}/web/lib/libwpcomm.la
%{_topdir}/BUILD/re-alpine-%{version}/web/lib/libwpcomm.so
%{_topdir}/BUILD/re-alpine-%{version}/web/lib/libwpcomm.so.1
%{_topdir}/BUILD/re-alpine-%{version}/web/lib/libwpcomm.so.1.0.0
...
%changelog
* Fri Apr 1 2011 Vaibhav Verma <vverna@nbcs.rutgers.edu> - 2.02-1.vanilla.ru
- Switched over from alpine to re-alpine (http://sourceforge.net/projects/re-alpine/)
- Removed all additional patches, and configuration settings to build a clean vanilla version. Check out alpine.spec in SVN for the older one.
- Added gettext as a dependency, and added -lintl to LDFLAGS to make the build work.
