...
Source: communicator-v477-us.sparc-sun-solaris2.5.1.tar.gz
...
%prep
%setup -q -n communicator-v477.sparc-sun-solaris2.5.1
...
# exit
EOF
cat <<EOF > $RPM_BUILD_ROOT/usr/local/bin/netscape-4.77
#!/bin/sh

REAL_NETSCAPE=/usr/local/netscape-4.77/netscape
MOZILLA_HOME=/usr/local/netscape-4.77
...
To use netscape's updater program, run:
    ...
    echo 'Communicator,4.77.0.20010326,/usr/local/netscape-4.77' > /tmp/infile
    vreg /usr/local/netscape-4.77/registry /tmp/infile
